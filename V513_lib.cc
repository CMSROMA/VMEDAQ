...
*/
  ...
  int ii;
  /* reset del modulo */
  if(V513_debug) printf("Entering init_V513 \n");

  address = V513_ADDRESS + io_shift.reset;
  DataLong = 0xff ;
  ...
  if(V513_debug) printf("IO reset: status = %d \n",status);
  
  /* setting del registro di STROBE */

  address = V513_ADDRESS + io_shift.strobe_reg ;
  ...
  for(ii=0;ii<V513_CHANNEL;ii++){
    //    chan_register = IO_CHAN_DIRECTION[ii] +
    chan_register = V513_CHAN_DIRECTION[ii] + V513_CHAN_POLARITY*0x2 +
    V513_CHAN_INPUT_MODE*0x4 + V513_CHAN_TRANSFER_MODE[ii]*0x8;
    DataLong = chan_register;
    address = V513_ADDRESS + 0x10 + 2*ii;
  ...
  if(V513_debug) printf("init_V513 exit status = %d \n",status);
  return(status);
}
...
unsigned short reset_daq(int32_t BHandle)
{
  ...
  address = V513_ADDRESS + 0x04; /* address of the write register */
  DataLong = 0x1; 
  //status = vme_write_dt(address, point2data, V513_AM,V513_DS);
  caenst = CAENVME_WriteCycle(BHandle,address,&DataLong,cvA24_U_DATA,cvD16);
  status *= (1-caenst); 
  DataLong = 0x0 ;
  //  status = vme_write_dt(address, point2data, V513_AM,V513_DS);
...
unsigned short read_trig(int32_t BHandle)
{
  ...
  address = V513_ADDRESS + 0x04; /* address of the write register */
  //  status = vme_read_dt(address, &value, V513_AM,V513_DS);
  caenst = CAENVME_ReadCycle(BHandle,address,&DataLong,cvA24_U_DATA,cvD16);
  status *= (1-caenst); 

  address = V513_ADDRESS + 0x48;
  //  status = vme_write_dt(address, &value, V513_AM,V513_DS);
  caenst = CAENVME_WriteCycle(BHandle,address,&DataLong,cvA24_U_DATA,cvD16);
  status *= (1-caenst); 
...
unsigned short busy_V513(int32_t BHandle, int ordine)
{
  int status(1),caenst;
  unsigned long address, DataLong;
...
vector<int> read_V513(int32_t BHandle, unsigned short *p_IO_value)
{
  vector<int> myD; myD.clear();
  unsigned short  status(1),caenst;
  unsigned long address, ValueRegister;

  address = V513_ADDRESS + 0x04; /*E' il registro dove e' conservato il 
  ...
  status *= (1-caenst); 
  if(V513_debug) printf("(read_ioreg) Reg Val :: %x\n",
...
unsigned short read_V513_old(int32_t BHandle, unsigned short *p_IO_value)
{
  ...
  //  status = vme_read_dt(address, &valueRegister,V513_AM,V513_DS);
  caenst = CAENVME_ReadCycle(BHandle,address,&DataLong,cvA24_U_DATA,cvD16);
  status *= (1-caenst); 
  if(V513_debug) printf("(read_V513) Reg Val :: %x\n",(unsigned int)DataLong);
...
unsigned short trigger_V513(int32_t BHandle)
{
  ...
  unsigned short status(1),caenst;
  //  printf("Entro in trigger_V513 \n");
   
  strobe_address = V513_ADDRESS+ 0x06; 
  /* Mette in strobeRegister il contenuto del registro di status */
  //  status = vme_read_dt(strobe_address, &strobeRegister, V513_AM,V513_DS); 
  ...
  printf("Esco in trigger_V513: status = %d \n", status);
  */
  return(status);
}
...
unsigned short clear_strobe_V513(int32_t BHandle)
{
  ...
  address = V513_ADDRESS+ 0x44; 
  /* V513_ADDRESS+ 0x44 individua il registro per pulire lo strobe. */
  
  DataLong = 0xff ;
  //  status= vme_write_dt(address, &data, V513_AM,V513_DS); 
  caenst = CAENVME_WriteCycle(BHandle,address,&DataLong,cvA24_U_DATA,cvD16);
  status *= (1-caenst); 
  return(status);
}
